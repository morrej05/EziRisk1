...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const payload = (await req.json()) as RemoveMemberPayload;
    if (!payload.organisation_id || !payload.target_user_id) {
      return new Response(JSON.stringify({ error: 'organisation_id and target_user_id are required' }), {
        status: 400,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
