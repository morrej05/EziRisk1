...
function normalizeModuleKey(key: string): string {
  // Extract RE## pattern if present
  const reMatch = key.match(/RE[_\s-]?(\d{2})/i);
  if (reMatch) {
    return `RE${reMatch[1]}`;
  }

  // Map legacy keys to canonical RE## codes
...
// Generate hazard/risk description using the same logic as the frontend
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { data: existingTemplates } = await supabase
      ...
      .select("title, observation, action_required");

    const existingKeys = new Set(
      (existingTemplates || []).map((t: any) => {
        const titleNorm = (t.title || "").toLowerCase().trim();
        const obsNorm = (t.observation || "").toLowerCase().trim();
        const actionNorm = (t.action_required || "").toLowerCase().trim();
        return `${titleNorm}|${obsNorm}|${actionNorm}`;
      })
    );
    ...
    for (const rec of recommendations as ReRecommendation[]) {
      ...
      // Normalize module key to canonical form (e.g., RE_03_OCCUPANCY → RE03)
      const normalizedModuleKey = normalizeModuleKey(rec.source_module_key);

      // Stronger deduplication check (title + observation + action_required)
      ...
      const key = `${titleNorm}|${obsNorm}|${actionNorm}`;
