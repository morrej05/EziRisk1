...
export function RecommendationWorkflowShell({
  ...
  return (
    <div className="space-y-4 rounded-xl border border-blue-100 bg-gradient-to-br from-white to-blue-50/30 p-5">
      <div className="flex flex-col gap-3 border-b border-blue-100 pb-4 lg:flex-row lg:items-start lg:justify-between">
        ...
        <div className="grid gap-2 text-xs sm:grid-cols-3 lg:min-w-[28rem]">
          <span className={`inline-flex items-center justify-center rounded-full border px-3 py-1 font-bold ${getPriorityBadgeClasses(priority)}`}>
            {priorityBand}
          </span>
          <span className="inline-flex items-center justify-center gap-1 rounded-full border border-blue-200 bg-blue-50 px-3 py-1 font-medium text-blue-700">
            <CalendarDays className="h-3.5 w-3.5" /> {targetDate || formatSuggestedCompletion(suggestedTimescale)}
          </span>
          <span className="inline-flex items-center justify-center gap-1 rounded-full border border-slate-200 bg-white px-3 py-1 font-medium text-slate-700">
