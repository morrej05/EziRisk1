...
export default function RE09RecommendationsForm({
  ...
  const addRecommendationFromLibrary = (libraryItem: any) => {
    ...
    const newRec: Recommendation = {
      ...
      source_module_key: libraryItem.related_module_key || moduleInstance.module_key,
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      for (const rec of recommendations) {
        const dedupeKey = `${rec.title.trim().toLowerCase()}::${(rec.observation_text || '').trim().toLowerCase()}::${(rec.action_required_text || '').trim().toLowerCase()}::${(rec.hazard_text || '').trim().toLowerCase()}::${moduleInstance.id}`;
        if (seenRecommendationKeys.has(dedupeKey)) {
          continue;
        }
        ...
        const recData = {
          ...
          source_module_key: rec.source_module_key || moduleInstance.module_key,
