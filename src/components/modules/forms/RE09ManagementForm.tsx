...
export default function RE09ManagementForm({
  ...
  const [formData, setFormData] = useState(() =>
    buildInitialFormData(moduleInstance.data || {}),
  );

  const [riskEngData, setRiskEngData] = useState<any>({});
  const [riskEngInstanceId, setRiskEngInstanceId] = useState<string | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    const hasPopulatedData = !!(
      (d.categories &&
        ...
        d.categories.some((c: any) => c.rating_1_5 !== null)) ||
      (d.recommendations && d.recommendations.length > 0) ||
      Object.keys(d).length > 0
    );

    const idChanged = lastIdRef.current !== moduleInstance.id;
    const transitionedToPopulated =
      hasPopulatedData && !seenPopulatedForCurrentIdRef.current;
  ...
  useEffect(() => {
    async function loadAutoRecommendationStates() {
      ...
      if (error) {
        console.error(
          "[RE09Management] Failed to load auto recommendation states:",
          error,
        );
        return;
      }
  ...
  const calculateOverallRating = (categories: any[]): number | null => {
    const rated = categories.filter(
      (c) => c.rating_1_5 !== null && c.rating_1_5 !== undefined,
    );
  ...
  const updateOverallRating = async (categories: any[]) => {
    ...
    try {
      const updatedRiskEngData = setRating(
        riskEngData,
        CANONICAL_KEY,
        overallRating,
      );
  ...
  const updateCategory = (key: string, field: string, value: any) => {
    ...
    const normalizedValue =
      field === "rating_1_5" && value !== null ? Number(value) : value;
    ...
    try {
      ...
      const managementPayload = {
        ...
        categories: formData.categories.map((cat: any) => {
          const uiRating =
            cat.rating_1_5 !== null ? Number(cat.rating_1_5) : null;
      ...
      const overallRating = calculateOverallRating(
        managementPayload.categories,
      );
      const syncCategoryAutos = async () => {
        let hasLifecycleChange = false;
        const lifecycleUpdates: Record<
          string,
          AutoRecommendationLifecycleState
        > = {};
  ...
  const overallRatingUI =
    overallRatingStored !== null
      ? Number(invertRatingForUI(overallRatingStored))
      : null;
  const overallRatingLabel =
    overallRatingUI !== null ? RATING_LABELS[overallRatingUI] : "Not rated";
      ...
      <div className="p-6 max-w-5xl mx-auto pb-24">
        <div className="mb-6">
          ...
          <p className="text-slate-600">
            Assessment of operational management and control systems
          </p>
        </div>

        <div className="bg-white rounded-lg border border-slate-200 p-6 mb-6">
          <h3 className="text-lg font-semibold text-slate-900 mb-3">
            Overall Management Systems Rating
          </h3>
        ...
        <div className="bg-white rounded-lg border border-slate-200 p-6 mb-6 space-y-6">
          <div>
            <h3 className="text-lg font-semibold text-slate-900 mb-4">
              Industry-Specific Risk Factors - Rating Panels
            </h3>
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-4">
              <p className="text-sm text-blue-900">
                <strong>Rating Scale:</strong> 1 = Poor/Inadequate, 2 = Below
                Average, 3 = Average/Acceptable, 4 = Good, 5 = Excellent. Poor
                ratings should typically trigger recommendations for
                improvement.
              </p>
            </div>
                ...
                >
                  ...
                  <div className="space-y-3">
                    <div>
                      ...
                      <RatingButtons
                        value={
                          category.rating_1_5 !== null
                            ? Number(category.rating_1_5)
                            : null
                        }
                        onChange={(num) =>
                          updateCategory(
                      ...
                      <p className="mt-2 text-xs text-slate-500">
                        {getAutoStateLabel(
                          category.rating_1_5 !== null
                            ? Number(category.rating_1_5)
                            : null,
                          autoRecStates[CATEGORY_FACTOR_KEYS[category.key]] ||
                            "none",
                        )}
                      </p>
                    </div>
                  ...
                  {document?.id && moduleInstance?.id && (
                    <ModuleActions
                      ...
                      sourceKey={`management_${category.key}`}
                      sourceLabel={CATEGORY_LABELS[category.key]}
                      compact
        ...
        {document?.id && moduleInstance?.id && (
          <ModuleActions
            documentId={document.id}
            moduleInstanceId={moduleInstance.id}
            buttonLabel="Add Module Recommendation"
            sectionKey="management_systems"
            sectionLabel="Management systems"
            sourceKey="management_systems_module_summary"
            sourceLabel="Management systems module summary"
            useInPlaceReRecommendationModal
          />
        )}
      </div>
